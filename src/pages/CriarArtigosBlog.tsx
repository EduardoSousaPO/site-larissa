// Componente temporário para criar os artigos iniciais do blog
// Execute esta página uma vez para adicionar os artigos ao Supabase
...
const CriarArtigosBlog = () => {
  const [status, setStatus] = useState<'idle' | 'criando' | 'sucesso' | 'erro'>('idle');
  const [mensagem, setMensagem] = useState('');

  const criarArtigos = async () => {
    setStatus('criando');
    setMensagem('Criando artigos...');
    ...
    try {
      ...
      if (existingPosts && existingPosts.length > 0) {
        setStatus('erro');
        setMensagem('Já existem posts no banco de dados. Os 5 artigos já foram criados anteriormente.');
        return;
      }
      ...
      for (let i = 0; i < artigos.length; i++) {
        ...
        if (error) {
          console.error(`Erro ao criar artigo ${i + 1}:`, error);
          throw new Error(`Erro ao criar artigo "${artigo.titulo}": ${error.message}`);
        }
        
        setMensagem(`Artigo ${i + 1} de ${artigos.length} criado: ${artigo.titulo}`);
      ...
      setStatus('sucesso');
      setMensagem(`Todos os ${artigos.length} artigos foram criados com sucesso! Agora você pode vê-los em /blog`);
    } catch (error: any) {
      console.error('Erro ao criar artigos:', error);
      setStatus('erro');
      setMensagem(error.message || 'Erro ao criar artigos. Verifique o console para mais detalhes.');
    }
  };
  ...
  return (
    <div className="container py-20">
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-8">
        <h1 className="text-3xl font-bold mb-6">Criar Artigos do Blog</h1>
        
        <p className="text-gray-700 mb-6">
          Este componente cria {artigos.length} artigos otimizados para SEO no blog.
          Clique no botão abaixo para adicioná-los ao Supabase.
        </p>

        <div className="mb-6">
          <h2 className="text-xl font-semibold mb-3">Artigos que serão criados:</h2>
          <ul className="list-disc list-inside space-y-2 text-gray-700">
            {artigos.map((artigo, index) => (
              <li key={index}>{artigo.titulo}</li>
            ))}
          </ul>
        </div>
        ...
        >
          {status === 'criando' ? 'Criando artigos...' : `Criar ${artigos.length} Artigos`}
        </button>

        {status === 'criando' && (
          <p className="mt-4 text-gray-600 text-center">{mensagem}</p>
        )}
      </div>
    </div>
  );
...
export default CriarArtigosBlog;
